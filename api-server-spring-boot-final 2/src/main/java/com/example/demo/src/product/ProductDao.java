...
//
//    }
    public GetProductPostRes getProducts() {
        ...
        return new GetProductPostRes(
                getProductInfo = this.jdbcTemplate.query(Query1,
                ...
                getProductImg = this.jdbcTemplate.query(Query2,
                (rs, nowNum) -> new GetProductImg(
                        rs.getString("firstImg"))
                ),

                getProductJuso = this.jdbcTemplate.query(Query5,
                        (rs, nowNum) -> new GetProductJuso(
                                rs.getString("jusoName"))
                ),

                getProductAttCount = this.jdbcTemplate.query(Query3,
                (rs, nowNum) -> new GetProductAttCount(
                        rs.getInt("attCount"))
                ),

                getProductChatCount = this.jdbcTemplate.query(Query4,
                (rs, nowNum) -> new GetProductChatCount(
                        rs.getInt("chatCount"))
                )
        );
